public class Solution {
    ...
    public boolean dfs(int[] nums, int start, int[] sum, int target) {
        if (start == nums.length)
            return sum[0] == target && sum[1] == target && sum[2] == target;//no need to check sum[3], once you do that, time limit exceed.
        boolean possible = false;
        for (int j = 0; j < sum.length; j++) {
            if (sum[j] + nums[start] <= target) {
                sum[j] += nums[start];
                possible = dfs(nums, start + 1, sum, target);
                if (possible)
                    return possible;
                sum[j] -= nums[start];
            }
...
public class Solution {
    ...
    public boolean dfs(int[] nums, int start, int[] sum, int target) {
        ...
        for (int j = 0; j < sum.length; j++) {
            if (sum[j] + nums[start] <= target) {
                sum[j] += nums[start];
                if (dfs(nums, start + 1, sum, target))
                    return true;
                sum[j] -= nums[start];
            }
        }
...
While accepted, this solution runs very slow (~450ms). Let's see what optimization can be done to improve it.
