...
class Solution {
    public int calculateMinimumHP(int[][] dungeon) {
        ...
        int[] dp = new int[n];
        for (int i = m - 1; i >= 0; i--) {
            dp[n - 1] = dp[n - 1] == 0 ? Math.max(1, 1 - dungeon[i][n - 1]) : Math.max(1, dp[n - 1] - dungeon[i][n - 1]);
            for (int j = n - 2; j >= 0; j--) {
                int min = Math.min(dp[j] > 0 ? dp[j] : dp[j + 1], dp[j + 1]) - dungeon[i][j];
                dp[j] = Math.max(1, min);
            }
        }
        return dp[0];
    }
}//start at [n - 1][m - 1] rather than [0][0]
...
Similarly, to satisfy the initial health of dungeon[i][j], the initial health of dungeon[i-1][j] (or dungeon[i][j-1]) should be at least initialHealth[i-1][j] + dungeon[i-1][j] = initialHealth[i][j], that is, initialHealth[i-1][j] = initialHealth[i][j] - dungeon[i-1][j]. 
