public class Solution {
    public int findMinArrowShots(int[][] points) {
        if (points == null || points.length <= 0)
            return 0;
        Arrays.sort(points, (x, y) -> (x[0] < y[0] ? -1 : x[0] == y[0] ? 0 : 1));
        int start = 0, end = Integer.MAX_VALUE, sum = 1;
        for (int[] point : points) {
            if(end < point[0]) {
                ...
                start = Math.max(point[0], start);
                end = Math.min(point[1], end);
            }
        }
        return sum;
    }
}//not very fast, will be better if sort point by point[1]
/*
