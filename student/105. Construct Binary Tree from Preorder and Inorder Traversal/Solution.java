...
public class Solution {
    ...
    public TreeNode buildTree(int preStart, int inStart, int inEnd, int[] preorder, int[] inorder) {
        ...
        for (int i = inStart; i <= inEnd; i++) {
            if (inorder[i] == root.val) {// could use a hashmap to replace this loop
                index = i;
                break;
            }
        }
        int leftTreeLen = index - inStart;// how to calculate the length of the left subtree
        root.left = buildTree(preStart + 1, inStart, index - 1, preorder, inorder);
        root.right = buildTree(preStart + leftTreeLen + 1, index + 1, inEnd, preorder, inorder);
        return root;
    }
}
/*
