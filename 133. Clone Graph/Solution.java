...
public class Solution {
    ...
    public UndirectedGraphNode dfs(UndirectedGraphNode node, Map<UndirectedGraphNode, UndirectedGraphNode> hash) {
        if (!hash.containsKey(node)) {
            hash.put(node, new UndirectedGraphNode(node.label));
            for (UndirectedGraphNode neighbor : node.neighbors)
                hash.get(node).neighbors.add(dfs(neighbor, hash));//dfs return node help to build the neighbors list as the same time when creating the nodes
        }
